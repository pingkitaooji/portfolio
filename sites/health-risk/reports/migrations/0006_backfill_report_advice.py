...
def backfill_report_advice(apps, schema_editor):
    report_model = apps.get_model("reports", "Report")
    for report in report_model.objects.all():
        if not report.medical_advice:
            report.medical_advice = [
                {
                    "risk_key": risk.get("key", ""),
                    "risk_name": risk.get("name", "風險項目"),
                    "level": risk.get("level", ""),
                    "recommendation": ADVICE_RULES.get(risk.get("key", ""), {}).get(
                        risk.get("level", ""),
                        "建議將本結果視為初步風險提示，並交由合格醫療專業人員搭配臨床資料解讀。",
                    ),
                    "pdf_recommendation": PDF_ADVICE_RULES.get(risk.get("key", ""), {}).get(
                        risk.get("level", ""),
        ...
        report.save(update_fields=["medical_advice", "disclaimer"])


class Migration(migrations.Migration):

    dependencies = [
        ("reports", "0005_report_advice_disclaimer"),
    ]

    operations = [
        migrations.RunPython(backfill_report_advice, migrations.RunPython.noop),
    ]
